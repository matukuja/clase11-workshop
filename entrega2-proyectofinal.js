...
function inicializarDOM() {
    ...
    botonDesloguearUsuario.onclick = desloguearUsuario
}

function desloguearUsuario() {
    localStorage.removeItem("clienteLogueado")
    labelLeyendaBienvenida.hidden = true
    botonDesloguearUsuario.hidden = true
...
function ingresoDeUsuario(event) {
    ...
    if ((inputNombre.value == "") || (inputApellido.value == "") || (inputDocumento.value == "") || (inputTelContacto.value == "")) {
        alert("Error al ingresar. Complete todos los datos por favor")
    } else {
    ...
    if (inputUnidades.value == "") {
        ...
        if ((equipoElegido == "") || (tipoProductoElegido == "") || (unidadesElegidas == 0)) {
            ...
            if (arrayPedidos.length > 0) {
                labelLeyendaCarritoVacio.hidden = true
            }
...
function pintarUsuarioLogueado(clienteAPintar) {
    let mensajeBienvenida = "Bienvenido, " + clienteAPintar.nombre + " " + clienteAPintar.apellido
    labelLeyendaBienvenida.innerText = mensajeBienvenida
    labelLeyendaBienvenida.hidden = false
    contenedorFormDatosPersonales.hidden = true
    contenedorDatosAdicionales.hidden = false
